class VigenereCipheringMachine {
  ...
  encrypt(message, key) {
      ...
      for (let i = 0; i < message.length; i++) {
          if (message[i] != ' ')
              j++
          encryptedMessage += (message[i].charCodeAt(0) > 64 && message[i].charCodeAt(0) < 91) ? String.fromCharCode((message.charCodeAt(i) + key.charCodeAt(j)) % 26 + 65) : message[i]
      }
  ...
  decrypt(encryptedMessage, key) {
      ...
      for (let i = 0; i < encryptedMessage.length; i++) {
          if (encryptedMessage[i] != ' ')
              j++
          message += (encryptedMessage[i].charCodeAt(0) > 64 && encryptedMessage[i].charCodeAt(0) < 91) ? String.fromCharCode((encryptedMessage.charCodeAt(i) + 26 - key.charCodeAt(j)) % 26 + 65) : encryptedMessage[i]
      }
